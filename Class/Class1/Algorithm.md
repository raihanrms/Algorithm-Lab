...
and finds the subset of the edges of that graph which
    ...
    2. has the minimum sum of weights among all the trees that can be formed from 
    the graph

It falls under a class of algorithms called greedy algorithms that find the local
optimum in the hopes of finding a global optimum. Start with the edges, that has the
lowest weight and keep adding edges until we reach our goal. The steps are:
...
creates a loop or not. The most common way to find this out is an algorithm called 
Union FInd. The Union-Find algorithm divides the vertices into clusters and allows us 
to check if two vertices belong to the same cluster or not and hence decide whether 
...
Step2: Choose the edge with the least weight, if there are more than 1, 
    choose anyone
